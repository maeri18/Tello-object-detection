...
class DetectAll(PluginBase):
    ...
    def __init__(self,name):
        ...
        self.subscriber1 = self.create_subscription(Image,self.image_raw_topic, self.sub1_callback,5)
        self.subscriber2 = self.create_subscription(Image,self.all_detected_topic, self.sub2_callback,5)
        self.cv2_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        self.video1 = cv2.VideoWriter('./img_raw.mp4',self.cv2_fourcc,20,(640,480))
        self.video2 = cv2.VideoWriter('./detected.mp4',self.cv2_fourcc,20,(640,480))
...
########################### Subscriber ###########################################################################################   
    def sub1_callback(self, img):
        ...
        self.video1.write(self.image_raw)
        

    def sub2_callback(self, img):
        """Callback function for the subscriber node (to topic /camera/image_raw).
        For each image received, it saves in the log that an image has been received.
        Then converts that image into cv2 format before performing object detection on that image and saving 
        the result in self.image_all_detected"""
        self.image_all_detected = self.cv_bridge.imgmsg_to_cv2(img, "bgr8")
        self.video2.write(self.image_all_detected)
